...
function processLastItem(stringList, callback) {
  const lastElement = stringList.length - 1;
  return callback(stringList[lastElement]);
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(items){
     runnerNames.push(`${items.last_name}, ${items.first_name}`);
  });
   return runnerNames;
}
...
function firstNamesAllCaps(runners) {
  const capsArray = [];
  runners.forEach(function(items){
     capsArray.push(items.first_name.toUpperCase());
  });
   return capsArray
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const getSize = runners.filter(function(item){
...
function tallyUpDonations(runners) {
  var reducer = (accumulator, item) => accumulator + item.donation;
  const donationTotals = runners.reduce(reducer, 0);
